...
import { ChevronLeft, ChevronRight, MapPin, Clock, X, Users, CheckCircle, Star, Calendar as CalendarIcon, Sparkles } from 'lucide-react';
...
export default function CalendarPage() {
  ...
  const [events, setEvents] = useState({
    '2026-02-21': { 
      ...
      type: 'Workshop',
      description: 'Learn the art of Korean tea ceremony',
      price: '₩35,000',
      capacity: 12,
      rating: 4.9,
      bookings: [
        { name: 'Kim Minho', email: 'kim@example.com', guests: 1 },
        { name: 'Park Sora', email: 'park@example.com', guests: 2 }
      ]
    },
    '2026-02-23': { 
      title: 'Suwon Hwaseong Fortress Night Tour',
      time: '19:00',
      location: 'Hwaseong Fortress, Suwon',
      type: 'Tour',
      description: 'Guided evening tour of UNESCO World Heritage site',
      price: '₩25,000',
      capacity: 20,
      rating: 4.8,
      bookings: [
        { name: 'Lee Junho', email: 'lee@example.com', guests: 1 }
      ]
    },
    '2026-02-28': { 
      title: 'K-Drama Filming Location Tour',
      time: '10:00',
      location: 'Gangnam & Bukchon, Seoul',
      type: 'Tour',
      description: 'Visit famous K-drama filming spots',
      price: '₩45,000',
      capacity: 15,
      rating: 5.0,
      bookings: []
    ...
    '2026-03-05': { 
      title: 'Korean Calligraphy Class',
      time: '15:00',
      location: 'Jongno-gu, Seoul',
      type: 'Workshop',
      description: 'Introduction to Korean brush calligraphy',
      price: '₩40,000',
      capacity: 10,
      rating: 4.7,
      bookings: []
    ...
    '2026-03-08': { 
      title: 'Traditional Market Food Tour',
      time: '11:00',
      location: 'Gwangjang Market, Seoul',
      type: 'Food Tour',
      description: 'Explore Seoul\'s oldest traditional market',
      price: '₩50,000',
      capacity: 8,
      rating: 4.9,
      bookings: []
  ...
  const typeColors = {
    'Tour': 'from-blue-500 to-blue-600',
    'Workshop': 'from-purple-500 to-purple-600',
    'Food Tour': 'from-orange-500 to-orange-600',
    'Performance': 'from-pink-500 to-pink-600',
    'Festival': 'from-green-500 to-green-600',
    'Exhibition': 'from-indigo-500 to-indigo-600'
  };
  ...
  const handleSaveEvent = () => {
    if (formData.title && formData.time && formData.location) {
      const existingBookings = events[selectedDate]?.bookings || [];
      setEvents({
        ...events,
        [selectedDate]: { 
          ...formData,
          ...
          <div className="bg-white rounded-2xl p-6 max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <div className="space-y-4">
              ...
              {events[selectedDate] && events[selectedDate].bookings && events[selectedDate].bookings.length > 0 && (
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">Current Bookings</label>
            ...
            <div className="flex gap-3 mt-6">
              <button
                ...
                className="flex-1 bg-gradient-to-r from-purple-600 to-pink-600 text-white py-3 rounded-lg hover:shadow-lg transition font-bold"
              >
                {events[selectedDate] ? 'Update Event' : 'Save Event'}
              </button>
              {events[selectedDate] && (
                <button
                  onClick={handleDeleteEvent}
                  className="px-6 bg-red-600 text-white py-3 rounded-lg hover:bg-red-700 transition font-bold"
                >
                  Delete
                </button>
              )}
            ...
            {!bookingSuccess ? (
              <>
                <div className="flex justify-between items-center mb-6">
                  ...
                  <button onClick={() => setShowBookingModal(false)} className="p-2 hover:bg-gray-100 rounded-lg transition">
                    <X size={20} />
                  </button>
                </div>
                ...
                <form onSubmit={handleSubmitBooking} className="space-y-4">
                  ...
                  <button
                    type="submit"
                    className="w-full bg-gradient-to-r from-purple-600 to-pink-600 text-white py-4 rounded-lg hover:shadow-lg transition font-bold text-lg"
                  >
                    Confirm Booking
                  </button>
                </form>
